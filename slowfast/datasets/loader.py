...
def construct_loader(cfg, split, is_precise_bn=False):
    ...
    Args:
        ...
        split (str): the split of the data loader. Options include `train`,
            `val`, and `test`.
    """
    assert split in ["train", "lab", "unl", "val", "test", "extract"]
    if split in ["train", "lab", "unl"]:
        ...
        dataset_name = cfg.TRAIN.DATASET
        batch_size = 3 * int(cfg.TRAIN.BATCH_SIZE / max(1, cfg.NUM_GPUS))
        shuffle = False
        drop_last = False
    elif split in ["test", "extract"]:
