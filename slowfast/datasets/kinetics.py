...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        for imdb_file in imdb_files:
            path_to_file = os.path.join(
                self.cfg.DATA.PATH_TO_DATA_DIR, imdb_file
            )
            ...
            with g_pathmgr.open(path_to_file, "r") as f:
                for clip_idx, path_label in enumerate(f.read().splitlines()):
                    ...
                    for idx in range(self._num_clips):
                        all_path_to_videos.append(
                            os.path.join(self.cfg.DATA.PATH_PREFIX, path)
                        )
                        all_labels.append(int(label))
                        all_spatial_temporal_idx.append(idx)
                        all_video_meta.append({})
                        all_imdb.append(
                            {"vid_path": os.path.join(self.cfg.DATA.PATH_PREFIX, path), 
                            "class": int(label)})
        
        random.seed(self.cfg.RNG_SEED)
        all_cases = list(range(len(all_path_to_videos)))
        ...
        self._dataset = []
        for idx in split_cases:
            self._dataset.append(
                (all_path_to_videos[idx], 
                all_labels[idx], 
                all_spatial_temporal_idx[idx], 
                all_video_meta[idx])
            )
    ...
    def __getitem__(self, index):
        ...
        path_to_video, label, spatial_temporal_idx, video_meta = self._dataset[index]
        ...
        for i_try in range(self._num_retries):
            ...
            if self.cfg.ADAPTATION.ENABLE and self.mode in ["train", "lab", "unl"]:
                ...
                frames = [weak_frames, strong_frames]
