...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        # This conversion is for the Swin transformer trained in the mmaction2 repo, which was saved differently.
        if any(x in checkpoint for x in ["state_dict", "model"]): #["meta", "state_dict", "optimizer"]):
            
            if "state_dict" in checkpoint:
                state_dict = checkpoint["state_dict"]
            elif "model" in checkpoint:
                state_dict = checkpoint["model"]
            revise_keys=[(r'^module\.', ''), (r'^backbone\.', ''), (r'^cls_head.fc_cls', 'head')]
            ...
            if 'meta' in checkpoint:
                checkpoint['epoch'] = checkpoint['meta']['epoch']
            if 'optimizer' in checkpoint:
                checkpoint['optimizer_state'] = checkpoint['optimizer']
