...
class AdaMeter(object):
    """
    Measure training stats.
    """

    def __init__(self, epoch_iters, cfg, pseudo_window=1000, dist_window=1000):
        """
        Args:
        ...
        self.num_top5_mis = 0
        self.num_samples = 0
        self.all_source_weak = []
        self.all_source_strong = []
        self.all_source_labels = []
        self.all_target_weak = []
        self.all_target_strong = []
        self.all_target_labels = []
        self.all_target_aligned = []
        self.all_pseudo_labels = [torch.tensor(0).cuda()]
        self.all_true_labels = [torch.tensor(0).cuda()]
        self.output_dir = cfg.OUTPUT_DIR
        self.pseudo_window = pseudo_window
    ...
    def update_predictions(self, source_weak, source_strong, source_labels, target_weak, target_strong, target_labels, target_aligned):
        """
        ...
        """
        if self.dist_window and len(self.all_source_labels)>=self.dist_window:
            self.all_source_weak = self.all_source_weak[1:]
            self.all_source_strong = self.all_source_strong[1:]
            self.all_source_labels = self.all_source_labels[1:]
            self.all_target_weak = self.all_target_weak[1:]
            self.all_target_strong = self.all_target_strong[1:]
            self.all_target_labels = self.all_target_labels[1:]
            self.all_target_aligned = self.all_target_aligned[1:]
        self.all_source_weak.append(source_weak)
        self.all_source_strong.append(source_strong)
        self.all_source_labels.append(source_labels)
        self.all_target_weak.append(target_weak)
        self.all_target_strong.append(target_strong)
        self.all_target_labels.append(target_labels)
        self.all_target_aligned.append(target_aligned)

    def update_pseudos(self, true_labels, pseudo_labels, mask):
        """
        Update pseudos labels.
        Args:
            pseudo_labels (tensor): all pseudo_labels.
            mask (tensor): mask on pseudo_labels to use.
        """
        true_labels = [l for l, m in zip(true_labels, mask) if m]
        pseudo_labels = [l for l, m in zip(pseudo_labels, mask) if m]
        if self.pseudo_window and len(self.all_pseudo_labels)>=self.pseudo_window:
            self.all_true_labels = self.all_true_labels[len(true_labels):]
            self.all_pseudo_labels = self.all_pseudo_labels[len(pseudo_labels):]
        self.all_true_labels.extend(true_labels)
        self.all_pseudo_labels.extend(pseudo_labels)
...
class AdaEmbedMeter(object):
    ...
    def __init__(self, epoch_iters, cfg):
        ...
        self.data_timer = Timer()
        self.net_timer = Timer()
        self.loss_s = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_t = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_s_total = 0.0
        self.loss_t_total = 0.0
        self.lr = None
        # Current minibatch errors (smoothed over a window).
        self.mb_top1_err = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_top5_err = ScalarMeter(cfg.LOG_PERIOD)
        ...
        self.all_unl_preds = []
        self.all_unl_labels = []
        self.all_unl_preds_refined = []
        self.all_pseudo_labels = [torch.tensor(0).cuda()]
        self.all_true_labels = [torch.tensor(0).cuda()]
        self.output_dir = cfg.OUTPUT_DIR
        self.pseudo_window = cfg.ADAPTATION.BANK_SIZE
        self.dist_window = cfg.ADAPTATION.BANK_SIZE
    ...
    def update_stats(self, top1_err, top5_err, loss_s, loss_t, lr, mb_size):
        """
        Update the current stats.
        Args:
            top1_err (float): top1 error rate.
            ...
            mb_size (int): mini batch size.
        """
        self.loss_s.add_value(loss_s)
        self.loss_t.add_value(loss_t)
        self.lr = lr
        self.loss_s_total += loss_s * mb_size
        self.loss_t_total += loss_t * mb_size
        self.num_samples += mb_size
    ...
    def update_predictions(self, lab_feats, lab_preds, lab_labels, unl_feats, unl_preds, unl_labels, unl_preds_refined):
        """
        Update predictions and labels.
        Args:
            preds (tensor): model output predictions.
            labels (tensor): labels.
        """
        if self.dist_window and len(self.all_unl_feats)>=self.dist_window:
            self.all_lab_feats = self.all_lab_feats[1:]
            self.all_lab_preds = self.all_lab_preds[1:]
            self.all_lab_labels = self.all_lab_labels[1:]
            self.all_unl_feats = self.all_unl_feats[1:]
            self.all_unl_preds = self.all_unl_preds[1:]
            self.all_unl_labels = self.all_unl_labels[1:]
            self.all_unl_preds_refined = self.all_unl_preds_refined[1:]
        self.all_lab_feats.append(lab_feats)
        self.all_lab_preds.append(lab_preds)
        self.all_lab_labels.append(lab_labels)
        self.all_unl_feats.append(unl_feats)
        self.all_unl_preds.append(unl_preds)
        self.all_unl_labels.append(unl_labels)
        self.all_unl_preds_refined.append(unl_preds_refined)
    ...
    def log_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "dt_net": self.net_timer.seconds(),
            "eta": eta,
            "loss_s": self.loss_s.get_win_median(),
            "loss_t": self.loss_t.get_win_median(),
            "lr": self.lr,
            "gpu_mem": "{:.2f}G".format(misc.gpu_mem_usage()),
        }
        if not self._cfg.DATA.MULTI_LABEL:
    ...
    def log_epoch_stats(self, cur_epoch):
        ...
        if not self._cfg.DATA.MULTI_LABEL:
            top1_err = self.num_top1_mis / self.num_samples
            top5_err = self.num_top5_mis / self.num_samples
            avg_loss_s = self.loss_s_total / self.num_samples
            avg_loss_t = self.loss_t_total / self.num_samples
            stats["top1_err"] = top1_err
            stats["top5_err"] = top5_err
            stats["loss_s"] = avg_loss_s
            stats["loss_t"] = avg_loss_t
        logging.log_json_stats(stats)
