...
def train_epoch(train_loader, model, optimizer, scaler, train_meter, cur_epoch, cfg, writer=None):
    ...
    batch_size = cfg.TRAIN.BATCH_SIZE * max(cfg.NUM_GPUS, 1)
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        train_meter.update_stats(top1_err, top5_err, loss, lr, batch_size)
...
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None):
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(val_loader):
        ...
        if cfg.DATA.MULTI_LABEL:
            ...
            val_meter.iter_toc()
            val_meter.update_stats(top1_err, top5_err, batch_size)
...
def clear_memory(inputs, labels, preds, loss):
