...
from slowfast.utils.meters import EpochTimer, TrainMeter, ValMeter, AdaEmbedMeter
...
def entropy(classifier, feat, lamda, eta=1.0):
    logits = classifier(feat, reverse=True, eta=-eta)
    preds = F.softmax(logits, dim=1)
    loss_ent = -lamda * torch.mean(torch.sum(preds *
                                             (torch.log(preds + 1e-5)), 1))
    return loss_ent
...
def adentropy(classifier, feat, lamda, eta=1.0):
    logits = classifier(feat, reverse=True, eta=eta)
    preds = F.softmax(logits, dim=1)
    loss_adent = lamda * torch.mean(torch.sum(preds *
                                              (torch.log(preds + 1e-5)), 1))
    return loss_adent
...
def train_epoch(
    ...
    backbone, classifier, momentum_backbone = models[0], models[1], models[2]
    optimizer_f, optimizer_c = optimizers[0], optimizers[1]

     # Enable train mode.
    backbone.train()
    classifier.train()
    momentum_backbone.train()
    ...
    for cur_iter, (inputs_source, labels_source, _, _) in enumerate(source_loader):
        ...
        # Load bank
        if len(train_meter.all_unl_feats) > 10:
            lab_feats_bank = torch.cat(train_meter.all_lab_feats, dim=0)
            unl_feats_bank = torch.cat(train_meter.all_unl_feats, dim=0)
            lab_probs_bank = torch.cat(train_meter.all_lab_preds, dim=0)
            unl_probs_bank = torch.cat(train_meter.all_unl_preds, dim=0)
            feats_bank = torch.cat((lab_feats_bank, unl_feats_bank), dim=0)
            probs_bank = torch.cat((lab_probs_bank, unl_probs_bank), dim=0)
        else:
            lab_feats_bank = torch.zeros(10, classifier.inc).cuda()
            unl_feats_bank = torch.zeros(10, classifier.inc).cuda()
        ...
        # Generate threshold.
        if cfg.ADAEMBED.PSEUDO_TYPE and len(train_meter.all_lab_preds) > 10:
            if cfg.ADAEMBED.PSEUDO_TYPE=='AdaMatch':
                c_tau = cfg.ADAEMBED.TAU * torch.ones(cfg.MODEL.NUM_CLASSES).cuda()
            elif cfg.ADAEMBED.PSEUDO_TYPE=='relative':
            ...
            elif cfg.ADAEMBED.PSEUDO_TYPE=='adaptive':
            ...
            elif cfg.ADAEMBED.PSEUDO_TYPE=='elastic':
                all_preds = F.softmax(torch.cat(train_meter.all_lab_preds, dim=0), dim=1).detach()
                max_preds = all_preds.max(dim=1)[0]
                class_preds = torch.argmax(all_preds, dim=1)
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            _, feats_strong = backbone([inputs_strong])
            logists_strong = classifier(feats_strong)
            with torch.no_grad():
                _, feats_weak = momentum_backbone([inputs_weak])
                logists_weak = classifier(feats_weak)
            ...
            if not cfg.ADAPTATION.SEMI_SUPERVISED.ENABLE:
                ...
                lab_feats = F.normalize(feats_slw, dim=1)
                lab_preds = logits_slw
                lab_labels = labels_source
                unl_feats = F.normalize(feats_tuw, dim=1)
                unl_preds = logits_tuw
                unl_labels = labels_target_unl
                lab_feats_strong = F.normalize(feats_sls, dim=1)
                unl_feats_strong = F.normalize(feats_tus, dim=1)
                lab_logits = logits_sls
            else:
                ...
                lab_feats = F.normalize(torch.cat((feats_slw, feats_tlw), dim=0), dim=1)
                lab_preds = torch.cat((logits_slw, logits_tlw), dim=0)
                lab_labels = torch.cat((labels_source, labels_target_lab), dim=0)
                unl_feats = F.normalize(feats_tuw, dim=1)
                unl_preds = logits_tuw
                unl_labels = labels_target_unl
                lab_feats_strong = F.normalize(torch.cat((feats_sls, feats_tls), dim=0), dim=1)
                unl_feats_strong = F.normalize(feats_tus, dim=1)
                lab_logits = torch.cat((logits_sls, logits_tls), dim=0)
            ...
            elif cfg.ADAEMBED.PSEUDO_TYPE=='AdaMatch+':
                preds_tuw_refined = F.normalize(alignment * preds_tuw, p=1, dim=1)
                preds_tuw_refined = F.normalize(preds_tuw_refined + (1 - alignment) * preds_tuw, p=1, dim=1)
            ...
            if cfg.ADAEMBED.LAMBDA_P:
                ...
                l_neg = torch.einsum("nc,ck->nk", [q, prototypes.T.clone().detach()])
            ...
            # Contrastive loss
            if cfg.ADAEMBED.LAMBDA_C:
                q = unl_feats_strong
                k = unl_feats
                l_pos = torch.einsum("nc,nc->n", [q, k]).unsqueeze(-1)
                l_neg = torch.einsum("nc,ck->nk", [q, unl_feats_bank.T.clone().detach()])
                logits_ins = torch.cat([l_pos, l_neg], dim=1) / cfg.ADAEMBED.TEMP
                labels_ins = torch.zeros(logits_ins.shape[0], dtype=torch.long).cuda()
            ...
            loss = loss_s + cfg.ADAEMBED.LAMBDA_T * mu * loss_t # + cfg.ADAEMBED.LAMBDA_P * mu * loss_p + cfg.ADAEMBED.LAMBDA_C * mu * loss_c
            loss.backward()
            optimizer_f.step()
            optimizer_c.step()
            ...
            _, feats_unl = backbone([target_unl_weak])
            loss_h = adentropy(classifier, feats_unl, 1.0)
            loss2 = cfg.ADAEMBED.LAMBDA * loss_h

            loss2.backward()
            optimizer_f.step()
            optimizer_c.step()
        
        for param_q, param_k in zip(backbone.parameters(), momentum_backbone.parameters()):
        ...
        if cfg.NUM_GPUS > 1:
            loss, loss_s, loss_t, loss_p, loss_c, loss_h, top1_err, top5_err = du.all_reduce(
                [loss.detach(), loss_s, loss_t, loss_p, loss_c, loss_h.detach(), top1_err, top5_err]
            )
...
def train(cfg):
    ...
    start_epoch = cu.load_train_checkpoint(cfg, backbone, optimizer_f, 
        scaler if cfg.TRAIN.MIXED_PRECISION else None)
    cu.load_train_checkpoint(cfg, momentum_backbone, None, None)
